    ...
    for (const p of res.rows) {
        ...
        rowsHtml += `
            <tr>
                ...
                <td>
                    ${esActivo ? `
                        <button onclick="abrirModalRenovarPortal('${p.id}')" class="btn-info" style="padding:4px 8px; font-size:12px;">🔄 Renovar (70h)</button>
...
async function abrirModalRenovarPortal(prestamoId) {
    try {
            ...
            itemsHtml += `
                <div style="display:flex; align-items:center; justify-content:space-between; padding:12px; border:1px solid #e2e8f0; border-radius:8px; margin-bottom:8px; background:${isDevuelto || tieneReserva ? '#f8fafc' : '#ffffff'};">
                    <div style="display:flex; align-items:center; gap:12px; flex:1;">
                        <input type="checkbox" class="chk-renovar-portal-item" value="${d.id}" ${isDevuelto || tieneReserva ? 'disabled' : 'checked'} style="width:18px; height:18px; cursor:pointer;">
                        <div>
            ...
            <div id="modal-renovacion-portal-overlay" class="modal-overlay">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button class="btn-secondary" onclick="cerrarModalRenovacionPortal()">Cancelar</button>
                        <button class="btn-info" onclick="procesarRenovacionPortalSeleccionados('${prestamo.id}', '${prestamo.fecha_devolucion_prevista}')" ${numRenovables === 0 ? 'disabled' : ''}>🔄 Confirmar Renovación</button>
                    </div>
                </div>
            </div>
        `;

        const existing = document.getElementById('modal-renovacion-portal-overlay');
...
function cerrarModalRenovacionPortal() {
    const el = document.getElementById('modal-renovacion-portal-overlay');
    if (el) el.remove();
}

async function procesarRenovacionPortalSeleccionados(prestamoId, fechaDevolucionActual) {
    const checkboxes = document.querySelectorAll('.chk-renovar-portal-item:checked:not([disabled])');
...
async function renovarPrestamoPortal(prestamoId, fechaDevolucionActual = null) {
    await abrirModalRenovarPortal(prestamoId);
}
